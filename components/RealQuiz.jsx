    ...
    <>
      ...
      <div className="fixed inset-0 -z-10 w-full h-full">
        ...
        <Image
          ...
          src={gradientBG}
          className="md:hidden object-cover opacity-15"
          fill
          priority
        />
        {/* Desktop Gradient */}
        <div
          className="hidden md:block w-full h-full opacity-50"
          style={{
            backgroundImage:
              "linear-gradient(345deg, hsl(0deg 0% 0%) 17%, hsl(0deg 100% 5%) 61%, hsl(0deg 100% 6%) 77%, hsl(5deg 100% 6%) 86%, hsl(11deg 100% 6%) 93%, hsl(17deg 100% 6%) 97%, hsl(21deg 100% 7%) 100%, hsl(22deg 82% 8%) 101%, hsl(21deg 69% 8%) 102%, hsl(19deg 58% 9%) 100%)",
      ...
      {QuizFinished ? (
        ...
        <div className="py-4">
          ...
          <ul className="absolute left-1/2 -translate-x-1/2 bottom-4 flex flex-col flex-wrap w-full px-4 gap-4 justify-center text-sm">
            <div
              className={`flex flex-row justify-center flex-wrap w-full gap-2 ${
                animateScore ? 'score-animate' : ''
              } ${BackanimateScore && 'back-score-animate'}`}
            >
              {shuffledAnswers.map((answer, index) => (
                <li key={index}>
                  <button
                    className={`transition duration-100 text-black font-bold py-2 px-4 rounded-[1.5rem] ${
                      selectedAnswers[currentQuestionIndex] === answer
                        ? 'bg-[#DEA78C]'
                        : 'bg-[#D0D0D0] hover:bg-[#B7B7B7]'
                    }`}
                    onClick={() => handleAnswer(answer, currentQuestionIndex)}
                  >
                    {answer}
                  </button>
                </li>
            ...
            <div className="flex justify-center gap-4 items-center text-center w-full mt-4 space-x-2">
              <div className="flex flex-row gap-2">
                <button
                  className={`transition duration-100 text-white font-bold p-2 rounded-full ${
                    currentQuestionIndex === 0 ? 'bg-transparent' : 'bg-gray-900 hover:bg-gray-800'
                  }`}
              ...
              <button
                className={`transition duration-200 rounded-[2rem] px-2 md:px-4 py-1 text-sm md:text-base md:py-2 font-bold ${
                  currentQuestionIndex != QuizQuestions.length - 1
                    ? 'bg-gray-800'
                    : 'hover:bg-[#AD7974] bg-[#B2847A]'
                }`}
                disabled={currentQuestionIndex != QuizQuestions.length - 1}
              >
                Submit
          ...
          <h2 className="h-14 absolute top-16 right-2 md:top-20 md:right-10 px-2 text-base sm:text-lg lg:text-xl font-bold">
            <div className="flex flex-row items-center gap-2">
              ...
              <ProgressBar
                ...
                totalQuestions1={QuizQuestions.length}
                currentScore={score}
              />
            </div>
          </h2>
          ...
          <div
            className={`${
              animateScore ? 'score-animate' : ''
            } ${BackanimateScore && 'back-score-animate'} border-l border-[#DEA78C] p-4 shadow-md`}
          >
            <h2
